...
%yyp = f(2)*yy + c(2)
%
%Reduced camera models: Currently manufactured cameras do not always justify this very general optical model. 
%For example, it now customary to assume rectangular pixels, and thus assume zero skew (alpha_c=0). 
%It is in fact a default setting of the toolbox (the skew coefficient not being estimated). 
%Furthermore, the very generic (6th order radial + tangential) distortion model is often not considered completely. 
%For standard field of views (non wide-angle cameras), it is often not necessary (and not recommended) to push 
%the radial component of distortion model beyond the 4th order (i.e. keeping kc(5)=0). This is also a default setting of the toolbox. 
%
...
if nargin < 6, % nargin is number of input parameters
   k = zeros(4,1);
   if nargin < 5,
      c = zeros(2,1);
      if nargin < 4,
         f = ones(2,1); % why here is ones?
         if nargin < 3,
            T = zeros(3,1);
            if nargin < 2,
               om = zeros(3,1);
...
[m,n] = size(X); % X is 3*156 

[Y,dYdom,dYdT] = rigid_motion(X,om,T); %Y: 3D coordinates of the structure points in the camera reference frame (3xN matrix for N points)
